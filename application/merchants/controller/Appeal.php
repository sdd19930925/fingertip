 ...
 */
class Appeal extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        $db = Db::name($this->table)
            ->alias("fa")
            ->field("fa.*,su.username")
            ->join("system_user su", "su.id=fa.uid");
        if (isset($get['ty']) && $get['ty'] == 2) {//客户端
            $db->where(["fa.uid" => session("user.id"), "fa.is_contact" => 2]);
        }
    ...
    public function _form_assign()
    {
        ...
        $this->assign(["apl" => $apl, "uid" => session("user.id")]);
    }
